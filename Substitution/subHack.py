...
# makeWordsPatterns.py, substitution.py, must exist in directory
import sys, os, re, copy, pprint, substitution, makeWordPatterns
...
def main():
    ...
    message = ''
    if len(sys.argv) == 1:
        message = raw_input("Enter your message to hack: ")
    else:
    ...
    if len(sys.argv) > 1:
        outFile = 'hacked.' + sys.argv[1]
        with open(outFile, 'w') as f:
            f.write(hackedMessage)
        print('Success! Hacked message written to: %s' % (outFile))
    else: print(hackedMessage)
